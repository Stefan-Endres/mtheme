MAKEFLAGS  := -j 1
INS         = source/beamerthemelantian.ins
...
DOC_SRC     = doc/lantiantheme.dtx
DOC_PDF     = doc/lantiantheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR     ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALL_DIR  = $(DESTDIR)/tex/latex/lantian
DOC_DIR      = $(DESTDIR)/doc/latex/lantian
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@lantian/\1@" -cf lantian-$(shell date "+%Y-%m-%d").tar.gz $(CTAN_CONTENT)
